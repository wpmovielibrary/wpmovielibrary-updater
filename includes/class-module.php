...
if ( ! class_exists( 'WPMOLY_Updater_Module' ) ) {

	/**
	 * Abstract class to define/implement base methods for all module classes
	 */
	abstract class WPMOLY_Updater_Module {
		...
		public static function render_template( $default_template_path = false, $variables = array(), $require = 'once', $admin = false ) {
			...
			$template_path = locate_template( 'wpmovielibrary-updater/' . $default_template_path, true, false );
			if ( ! $template_path )
				$template_path = WPMOLY_UPDATER_PATH . 'views/' . $default_template_path;
